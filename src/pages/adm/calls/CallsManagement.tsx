...
type WorkflowState = Record<PermissionKey, boolean>
type WorkflowMatrix = Record<StateKey, WorkflowState>
type WorkflowByCall = Record<string, WorkflowMatrix>
...
export default function CallWorkflow() {
  ...
  const [workflowByCall, setWorkflowByCall] = useState<WorkflowByCall>(() => ({
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
      <Helmet>
        <title>Gest • PROPESQ</title>
      </Helmet>

      <Link
        to="/adm/calls/CreateCall"
        className="inline-flex items-center gap-2 rounded-full border border-neutral-light bg-white px-4 py-2 text-sm font-semibold text-primary hover:bg-neutral-50 transition-colors w-fit"
      >
        <ArrowLeft size={16} />
        Voltar para 
      </Link>
